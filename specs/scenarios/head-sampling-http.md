...
Each language implementation lives under `scenarios/head-sampling-http/<language>/` and ships **one service image** (one Dockerfile) reused by every deployment below.
...
Reference implementations: [`scenarios/head-sampling-http/nodejs/`](../../scenarios/head-sampling-http/nodejs/), [`scenarios/head-sampling-http/python/`](../../scenarios/head-sampling-http/python/).
...
A complete `head-sampling-http` topology has **four Deployments** (plus optional Jobs and Sampling CRDs). Names below match the Node.js reference; other languages use the same deployment names with a language-specific namespace (e.g. `nodejs-head-sampling`).

E2E: [`scenarios/head-sampling-http/test/chainsaw-test.yaml`](../../scenarios/head-sampling-http/test/chainsaw-test.yaml).
...
```
scenarios/head-sampling-http/<language>/
  ...
  helm-values.yaml              # Odigos Source manifests (via ../helm)
  Makefile                      # build, apply all manifests, curl job targets
```

Apply order (reference): create app namespace → apply all Deployment/Service manifests → apply Odigos Sources via `make apply-odigos-sources` → apply Sampling CRDs in `odigos-system`.
